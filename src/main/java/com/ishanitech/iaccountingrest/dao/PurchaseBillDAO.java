...
public interface PurchaseBillDAO {
        ...
        @SqlQuery("   select * FROM purchase_bill b WHERE status = true and purchase_bill_no = :id")
        @RegisterBeanMapper(PurchaseBillDTO.class)
        PurchaseBillDTO getSinglePurchaseBill(Integer id);

        // @GetGeneratedKeys
        // @SqlUpdate("INSERT INTO purchase_bill (user_id, cust_id, company_id,
        // invoice_no, fiscal_year, total, tax, discount, grand_total)" +
        // " VALUES (:userId, :custId, :companyId, :invoiceNo, :fiscalYear, :total,
