...
public interface ProductDAO {
        ...
        @GetGeneratedKeys
        @SqlUpdate("INSERT INTO product (name, description, selling_price, cost_price, user_id, company_id, branch_id, seller_id, category_id, barcode, discount ,tax,unit ,quantity_per_unit)"
                        +
                        "VALUES ( :name, :description, :sellingPrice, :costPrice, :userId, :companyId, :branchId, :sellerId, :categoryId, :barcode, :discount ,:tax,:unit ,:qtyPerUnit)")
        ...
        @SqlQuery("""
                        ...
                        p.company_id as company_id, p.seller_id as seller_id, p.category_id as category_id, p.create_date as create_date, p.update_date
                        as update_date, p.barcode as barcode, p.discount as discount, p.tax as tax ,p.unit as unit FROM product p WHERE p.deleted = false and p.id in (<newProdIds>)
                        """)
        @RegisterBeanMapper(ProductDTO.class)
